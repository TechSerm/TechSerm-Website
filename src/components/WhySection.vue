<template>
    <section id="why">
        <div class="">
            <div class="flex flex-col lg:flex-row items-center">
                <!-- Image on Left -->
                <div class="flex justify-center">
                    <div class="w-full lg:w-6/8 md:w-5/8 mb-10 lg:mb-0 lg:pr-10">
                        <img src="/src/assets/img/header_image.png" alt="About Us"
                            class="w-full max-w-[600px] h-auto object-cover" loading="lazy">
                    </div>
                </div>

                <!-- Content on Right -->
                <div class="w-full lg:w-1/2">
                    <div class="">
                        <div class="mb-8">
                            <span class="techserm-text-blue font-semibold tracking-wide">WHY CHOOSE US</span>
                            <h2 class="text-3xl md:text-4xl font-bold mt-2 mb-4 techserm-text-black">Your Goal is Our
                                Achievement</h2>
                            <p class="text-gray-600 leading-relaxed">
                                Nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat sed diam voluptua.
                                At vero eos et accusam et justo duo dolores et rebum.
                            </p>
                        </div>

                        <ul class="space-y-4">
                            <li class="flex items-start">
                                <div class="flex-shrink-0">
                                    <div
                                        class="flex items-center justify-center h-6 w-6 rounded-full techserm-bg-blue text-white">
                                        <i class="fas fa-check text-xs"></i>
                                    </div>
                                </div>
                                <div class="ml-3">
                                    <p class="text-gray-700">Vero eos et accusam et justo duo dolores et rebum.</p>
                                </div>
                            </li>
                            <li class="flex items-start">
                                <div class="flex-shrink-0">
                                    <div
                                        class="flex items-center justify-center h-6 w-6 rounded-full techserm-bg-blue text-white">
                                        <i class="fas fa-check text-xs"></i>
                                    </div>
                                </div>
                                <div class="ml-3">
                                    <p class="text-gray-700">At vero eos et accusam et justo duo dolores et rebum.</p>
                                </div>
                            </li>
                            <li class="flex items-start">
                                <div class="flex-shrink-0">
                                    <div
                                        class="flex items-center justify-center h-6 w-6 rounded-full techserm-bg-blue text-white">
                                        <i class="fas fa-check text-xs"></i>
                                    </div>
                                </div>
                                <div class="ml-3">
                                    <p class="text-gray-700">Dvero eos et accusam et justo duo dolores et rebum.</p>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>